...
# Using a different http port in test to allow running tests while the iex shell
# session is running.
http_port = 
  if config_env() != :test,
...
db_folder = 
  if config_env() != :test,
    ...
    else: System.get_env("TODO_DB_FOLDER", "./test_persist")
  
config :todo, :database, db_folder: db_folder

# Using a shorter to-do server expiry in local defoverridable
todo_server_expiry = 
  if config_env() != :dev,
